...
export const parseInput = (input: ReadonlyArray<string>): [Map<string, number>, Map<string, number>, ReadonlyArray<Template>] => {
...
export const applyTemplate = (templates: ReadonlyArray<Template>, countMap: Map<string, number>) => (map: Map<string, number>): Map<string, number> => {
    ...
    for (let i = 0; i < templates.length; i++) {
        ...
        if (map.has(template.pair)) {
            ...
            nextMap.set(pair1, (nextMap.get(pair1) ?? 0) + (map.get(template.pair) as number));
            nextMap.set(pair2, (nextMap.get(pair2) ?? 0) + (map.get(template.pair) as number));
            countMap.set(template.insertion, (countMap.get(template.insertion) ?? 0) + (map.get(template.pair) as number));
        }
    }
