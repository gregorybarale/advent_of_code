...
export const isNumberInInterval =
  (interval: Interval) => (n: number): boolean =>
    interval[0] <= n && n <= interval[1];
export const doesIntervalOverlap =
  (intervalA: Interval) => (intervalB: Interval): boolean =>
    isNumberInInterval(intervalA)(intervalB[0]) ||
    isNumberInInterval(intervalA)(intervalB[1]);

export const parseInput = (input: ReadonlyArray<string>): ReadonlyArray<Step> =>
  input.map((str) => {
    const regexp =
      /(on|off) x=(-?\d+)\.\.(-?\d+),y=(-?\d+)\.\.(-?\d+),z=(-?\d+)\.\.(-?\d+)/;
    if (!regexp.test(str)) throw new Error("Bad step parsing");
    const [_, isOn, xStart, xEnd, yStart, yEnd, zStart, zEnd] = regexp.exec(
      str,
    ) as RegExpExecArray;
    return {
      isOn: isOn === "on",
...
export const applyStepOnCore = (step: Step) => (core: Core): Core =>
  core.map((cube) => {
    if (
...
export const doesCuboidIntersect =
  (cuboid1: Cuboid) => (cuboid2: Cuboid): boolean => {
