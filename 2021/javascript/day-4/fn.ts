...
import { parseInput, checkBoard, Board, getUnmarkedNumbers } from './utils.ts';
...
export const fn1 = ({ input }: IAoCInput) => {
    ...
    const sumOfUnmarked = getUnmarkedNumbers(numbers)(winningBoard).reduce((acc, n) => acc + n, 0);
...
export const fn2 = ({ input }: IAoCInput) => {
    ...
    while (!lastWinningBoard) {
        numbers = drawnedNumbers.filter((_, i) => i <= counter);
        const winningBoards = boards.filter(board => checkBoard(numbers)(board));
        tempWinningBoard = [...tempWinningBoard, ...winningBoards.filter(board => !tempWinningBoard.includes(board))];
    ...
    const sumOfUnmarked = getUnmarkedNumbers(numbers)(lastWinningBoard).reduce((acc, n) => acc + n, 0);
    return sumOfUnmarked * ([...numbers].pop() as number);
};
