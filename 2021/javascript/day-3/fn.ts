...
export const fn1 = ({ input }: IAoCInput) => {
  ...
  const gammaRate = transformMergedReportToBit(report.length, true)(
    mergedReport,
  ).join("");
  const epsilonRate = transformMergedReportToBit(report.length, false)(
    mergedReport,
  ).join("");
...
export const fn2 = ({ input }: IAoCInput) => {
  ...
  return Number.parseInt(oxygenGeneratorRating, 2) *
