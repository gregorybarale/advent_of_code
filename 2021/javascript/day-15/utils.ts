...
export const parseInput = (input: ReadonlyArray<string>): CavernCeiling => input.reduce((acc, row, y) => {
    return [...acc, ...row.split('').map((str, x) => ({
        x,
        y,
...
export const printCeiling = (ceiling: CavernCeiling): void => {
    ...
    for (let i = 0; i <= maxY; i++) {
        ...
        for (let j = 0; j <= maxX; j++) {
            row = row + (ceiling.find(({ x, y }) => x === j && y === i)?.riskLevel ?? ".");
        }
        console.log(row);
    }
    console.log('---');
