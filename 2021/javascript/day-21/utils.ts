...
export const gameToKey = (game: Game): string =>
  `${game[0].id}-${game[0].position}-${game[0].score}|${game[1].id}-${
    game[1].position
  }-${game[1].score}`;
...
export const rollDeterministicDiceThreeTimes = (
  oldState: number,
): [number, number] => {
  return [oldState * 3 + 3, oldState + 3];
};

export const hasGameEnded = (maxScore: number) => (game: Game) =>
  game.some(({ score }) => score >= maxScore);

export const isAllUniversesEnded =
  (maxScore: number) => (universes: Map<string, number>) =>
    [...universes.keys()].every((key) =>
      hasGameEnded(maxScore)(keyToGame(key))
    );

export const runQuanticTurn =
  (maxScore: number) =>
  (universes: Map<string, number>) =>
  (playerId: 1 | 2): Map<string, number> => {
...
export const getPlayerNumberOfWin =
  (maxScore: number) =>
  (playerId: 1 | 2) =>
  (universes: Map<string, number>): number =>
    [...universes.entries()]
      .filter(([key, value]) =>
        (keyToGame(key).find(({ id }) => id === playerId)?.score as number) >=
          maxScore
      )
