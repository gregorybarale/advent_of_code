...
export const getAdjacents =
  (population: ReadonlyArray<IDumboOctopus>) =>
  ({ x, y }: IDumboOctopus): ReadonlyArray<IDumboOctopus> =>
    population.filter((dumbo) => {
...
export const hasDumboReadyToFlash = (
  dumboPopulation: ReadonlyArray<IDumboOctopus>,
): boolean =>
  dumboPopulation.some(({ energyLevel, hasFlashedInStep }) =>
    energyLevel > 9 && !hasFlashedInStep
  );

export const runStep = (
  dumboPopulation: ReadonlyArray<IDumboOctopus>,
): [ReadonlyArray<IDumboOctopus>, number] => {
  const nextPopulationState: ReadonlyArray<IDumboOctopus> = dumboPopulation.map(
    (dumbo) => ({
      ...dumbo,
  ...
  while (hasDumboReadyToFlash(nextPopulationState)) {
    const nextToFlash = nextPopulationState.filter((
      { energyLevel, hasFlashedInStep },
    ) => energyLevel > 9 && !hasFlashedInStep);
  ...
  return [
    nextPopulationState.map((dumbo) => ({
      ...dumbo,
      energyLevel: dumbo.hasFlashedInStep ? 0 : dumbo.energyLevel,
      hasFlashedInStep: false,
    })),
    nextPopulationState.filter(({ hasFlashedInStep }) => hasFlashedInStep)
      .length,
  ];
};
...
export const print = (population: ReadonlyArray<IDumboOctopus>) => {
  for (let x = 0; x < Math.sqrt(population.length); x++) {
    ...
    for (let y = 0; y < Math.sqrt(population.length); y++) {
      row = row +
        population.find(({ x: posX, y: posY }) => posX === x && posY === y)
          ?.energyLevel;
    }
    console.log(row);
  }
