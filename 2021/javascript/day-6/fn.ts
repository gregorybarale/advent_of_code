...
export const fn1 = ({ input }: IAoCInput) => {
    let day = 0;
    let populationState: ReadonlyArray<number> = input[0].split(',').map(s => Number.parseInt(s, 10));
    while (day < 80) {
        day++;
...
export const fn2 = ({ input }: IAoCInput) => {
    let day = 0;
    const inputParsed: ReadonlyArray<number> = input[0].split(',').map(s => Number.parseInt(s, 10));
