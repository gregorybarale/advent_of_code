export const runDayNaive = (
  populationPreviousState: ReadonlyArray<number>,
...
export const runDayPro = (
  populationPreviousState: Map<number, number>,
): Map<number, number> => {
  ...
  [...populationPreviousState.entries()].forEach(([key, value]) => {
    if (key === 0) {
      populationNextState.set(
        6,
        (populationNextState.get(6) as number) + value,
      );
