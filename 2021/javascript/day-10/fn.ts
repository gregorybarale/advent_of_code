...
export const fn1 = ({ input }: IAoCInput) => {
  ...
  const firstWrongChars = lines.map(getFirstInvalidChar).filter((c) =>
    c !== undefined
  );
...
export const fn2 = ({ input }: IAoCInput) => {
  const lines = parseInput(input);
  const missingChars = lines.filter((line) =>
    getFirstInvalidChar(line) === undefined
  ).map(getMissingChars);
