...
export const isOpeningChunk = (s: ValidChar) =>
  s === "(" || s === "[" || s === "{" || s === "<";
...
): ReadonlyArray<ReadonlyArray<ValidChar>> =>
  input.map((s) => s.split("") as ReadonlyArray<ValidChar>);

export const getFirstInvalidChar = (
  line: ReadonlyArray<ValidChar>,
): ValidChar | undefined => {
...
export const getMissingChars = (
  line: ReadonlyArray<ValidChar>,
): ReadonlyArray<ClosingChar> => {
  const stack: Array<ValidChar> = [];
  for (let i = 0; i < line.length; i++) {
    const nextChar = line[i];
    if (isOpeningChunk(nextChar)) {
...
export const computeSyntaxErrorCode = (
  errors: ReadonlyArray<ClosingChar>,
): number =>
...
export const computeMissingCode = (
  missings: ReadonlyArray<ClosingChar>,
): number =>
  missings.reduce((acc, missing) => {
