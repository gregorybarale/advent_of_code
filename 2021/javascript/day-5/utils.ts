...
export const isPositionsEqual = (a: IPosition, b: IPosition) => a.x === b.x && a.y === b.y;

export const computeVerticalPositions = (partialLine: Partial<ILine>): ReadonlyArray<IPosition> => {
    if (!Boolean(partialLine?.start) || !Boolean(partialLine?.end)) throw new Error('[computeVerticalPositions] Missing Data');
    const y = (partialLine.start as IPosition).y;
    const min = Math.min((partialLine.start as IPosition).x, (partialLine.end as IPosition).x);
    const max = Math.max((partialLine.start as IPosition).x, (partialLine.end as IPosition).x);
...
export const computeHorizontalPositions = (partialLine: Partial<ILine>): ReadonlyArray<IPosition> => {
    if (!Boolean(partialLine?.start) || !Boolean(partialLine?.end)) throw new Error('[computeHorizontalPositions] Missing Data');
    const x = (partialLine.start as IPosition).x;
    let positions: ReadonlyArray<IPosition> = [];
    const min = Math.min((partialLine.start as IPosition).y, (partialLine.end as IPosition).y);
    const max = Math.max((partialLine.start as IPosition).y, (partialLine.end as IPosition).y);
    for (let i = min; i <= max; i++) {
        positions = [...positions, {
            x,
            y: i,
...
export const computeDiagonalPositions = (partialLine: Partial<ILine>): ReadonlyArray<IPosition> => {
    let positions: ReadonlyArray<IPosition> = [];
    let currentPosition: IPosition = partialLine.start as IPosition;
    const changeX = (partialLine.start as IPosition).x < (partialLine.end as IPosition).x ?
        (x: number) => x + 1 : (x: number) => x - 1;
    const changeY = (partialLine.start as IPosition).y < (partialLine.end as IPosition).y ?
...
export const parseInput = (input: ReadonlyArray<string>): ReadonlyArray<ILine> => {
