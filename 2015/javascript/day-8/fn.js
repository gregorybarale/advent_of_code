...
const fn1 = input => {
  const max = txtToArray(input)[0].replace(/\\"/g, '##').replace(/\\\\/g, '##').replace(/(\\x([0-9]|[a-f]|[A-F]){2})/g, '####').length;
...
console.log(txtToArray('./input.txt')[0].replace(/\\"/g, '##').replace(/\\\\/g, '##').replace(/(\\x([0-9]|[a-f]|[A-F]){2})/g, '####').length);
