...
const checkIfAstroidBetween =
  (astroidOrigin, astroidEnd) => (astroidToTest) => {
    ...
    if (
      astroidOrigin.x === astroidToTest.x && astroidOrigin.y === astroidToTest.y
    ) {
      throw new Error(
        "[checkIfAstroidBetween] Astroid to test and End must be different",
      );
    }
    if (astroidToTest.x === astroidEnd.x && astroidToTest.y === astroidEnd.y) {
      throw new Error(
        "[checkIfAstroidBetween] Origin and Astroid to test must be different",
      );
    }
    ...
    const coefficient = (astroid.y - astroidOrigin.y) /
      (astroid.x - astroidOrigin.x);
    const offset = astroid.y -
      (astroid.y - astroidOrigin.y) / (astroid.x - astroidOrigin.x) * astroid.x;
...
function getAstroidsInSight(astroids, astroidOrigin) {
  ...
  for (let i = 0; i < astroids.length; i++) {
    ...
    const hasAstroidsBetween = astroids.some((astroid) =>
      astroid.x !== astroidOrigin.x &&
...
const fn1 = (input) => {
  const astroids = getAstroidCoordonate(input).map(
    (astroid, _, astroidsArray) => {
