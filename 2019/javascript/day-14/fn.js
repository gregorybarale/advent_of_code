...
const fn1 = (reactionsAsString) => {
  const reactions = reactionsAsString.map((reactionAsString) =>
    reactionParser(reactionAsString)
  );
  const oreReactions = reactions.filter((reaction) =>
    reaction.inputs.some((input) => input.id === "ORE")
  );
  const othersReactions = reactions.filter((reaction) =>
    reaction.inputs.every((input) => input.id !== "ORE")
  );

  const elementsFromORE = oreReactions.map((reaction) => reaction.output.id);

  let aggregatedReaction = {
    ...othersReactions.find((reaction) => reaction.output.id === "FUEL"),
  };

  const nextAggregatedReaction = {
    output: aggregatedReaction.output,
  };
  const inputsFromOre = aggregatedReaction.inputs.filter((input) =>
    elementsFromORE.includes(input.id)
  );
  const inputsToTransform = aggregatedReaction.inputs.filter((input) =>
    !elementsFromORE.includes(input.id)
  );
